...
fs.readdirSync('./src/view/theme').forEach(filename => {
    if (fs.lstatSync('./src/view/theme/' + filename).isFile()) {
        plugins.push(
            new HtmlWebpackPlugin(Object.assign({}, htmlPluginOptions, {
                template: './src/view/theme/' + filename,
                filename: filename.substring(0, filename.length - 4) + '.template'
            }))
        );
    }
});
