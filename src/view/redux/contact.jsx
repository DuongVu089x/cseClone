...
export function getConferenceContactInPage(pageNumber, pageSize, done) {
    return (dispatch, getState) => {
        const page = T.updatePage('adminContact', pageNumber, pageSize),
            doGetContact = () => {
                const state = getState();
                if (state.conference && state.conference.item) {
                    let url = '/admin/contact/conference/' + state.conference.item._id + '/page/' + page.pageNumber + '/' + page.pageSize;
                    T.get(url, data => {
                        if (data.error) {
                            T.notify('Lấy danh sách liên hệ bị lỗi!', 'danger');
                            console.error('GET: ' + url + '. ' + data.error);
...
export function updateConferenceContact(_id, changes, done) {
    return dispatch => {
        const url = '/admin/contact';
        T.put(url, { _id, changes }, data => {
            if (data.error) {
                ...
                T.notify('Cập nhật thông tin liên hệ thành công!', 'info');
                dispatch(getConferenceContactInPage());
                done && done();
            }
        }, error => T.notify('Cập nhật thông tin liên hệ bị lỗi!', 'danger'));
    }
...
export function deleteConferenceContact(_id) {
    return dispatch => {
        const url = '/admin/contact';
        T.delete(url, { _id }, data => {
            if (data.error) {
...
export function getAuthorContacts(done) {
    return (dispatch, getState) => {
        const doGetAuthorContacts = () => {
            const state = getState();
            if (state && state.conference && state.conference.item) {
                const url = '/author/contact/all/' + state.conference.item._id;
                T.get(url, data => {
                    if (data.error) {
                        T.notify('Error when get all messages of current user!', 'danger');
                        console.error('GET: ' + url + '. ' + data.error);
                    } else {
                        if (done) done(data.items);
                        dispatch({ type: GET_CONTACTS, items: data.items });
                    }
                }, error => T.notify('Error when get all messages of current user!', 'danger'));
            } else {
                setTimeout(doGetAuthorContacts, 500);
            }
        };
        doGetAuthorContacts();
    };
...
export function authorSendMessage(contact, done) {
    return dispatch => {
        const url = '/author/contact';
        T.post(url, { contact }, data => {
            if (data.error) {
                T.notify('Error when send message!', 'danger');
                console.error('POST: ' + url + '. ' + data.error);
            } else {
                dispatch(getAuthorContacts(done));
            }
        }, error => T.notify('Error when send message!', 'danger'));
    }
}

export function deleteAuthorContact(_id, done) {
    return dispatch => {
        const url = '/author/contact';
        T.delete(url, { _id }, data => {
            if (data.error) {
                T.notify('Error when delete message!', 'danger');
                console.error('DELETE: ' + url + '. ' + data.error);
            } else {
                dispatch(getAuthorContacts(done));
                T.alert('Message deleted successfully!', 'error', false, 800);
            }
        }, error => T.notify('Error when delete message!', 'danger'));
    }
}