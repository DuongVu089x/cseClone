    ...
    return dispatch => {
        T.post('/login', data, res => {
            if (res.error) {
                ...
                if (res.user) {
                    const role = res.user.role.toString().trim().toLowerCase();
        ...
        T.confirm(config.title, config.message, true, isConfirm => {
            isConfirm && T.post('/logout', {},
                data => {
                    ...
                    if (pathname.startsWith('/admin') || pathname.startsWith('/editor') || pathname.startsWith('/author') || pathname.startsWith('/user')) {
                        window.location = '/';
                    } else if (config.done) {
                        config.done();
                    }
