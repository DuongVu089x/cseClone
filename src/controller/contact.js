module.exports = app => {
    ...
    app.get('/admin/contact/conference/:conferenceId/page/:pageNumber/:pageSize', app.role.isAdmin, (req, res) => {
        const pageNumber = parseInt(req.params.pageNumber),
            pageSize = parseInt(req.params.pageSize);
        app.model.contact.getPage(pageNumber, pageSize, { conferenceId: req.params.conferenceId }, (error, page) => {
            page.list = page.list.map(item => app.clone(item, { message: '' }));
            res.send({ error, page });
        });
    });

    app.get('/admin/contact/all', app.role.isAdmin, (req, res) => app.model.contact.getAll((error, items) => res.send({ error, items })));
    ...
    // Home -----------------------------------------------------------------------------------------------------------------------------------------
    app.post('/home/contact', (req, res) => app.model.contact.create(req.body.contact, (error, item) => {
        if (item) {
